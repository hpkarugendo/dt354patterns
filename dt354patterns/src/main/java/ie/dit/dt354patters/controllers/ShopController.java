...
public class ShopController {
    ...
    public String addCartItem(@PathVariable Integer id, Model model, HttpServletRequest req){
	Account acc = AccountResolver.INSTANCE.getAccount(req);
	Customer thisCustomer = cRepo.findByEmail(acc.getEmail());
	BookOrder bo = boRepo.findByCustomerAndNewOrder(thisCustomer, true);
	Book b = bRepo.findOne(id);
	if(bo == null){
	    bo = new BookOrder();
	    bo.setNewOrder(true);
	    bo.setCustomer(thisCustomer);
	    BookOrder anotherBo = boRepo.save(bo);
	    BookItem bi = new BookItem();
	    bi.setBook(b);
	    bi.setQuantity(1);
	    anotherBo.getItems().add(bi);
	    anotherBo.setOrderTotal(anotherBo.getOrderTotal() + bi.getBook().getPrice());
	    boRepo.save(anotherBo);
	} else {
	    boolean found = false;
	    for(BookItem bit: bo.getItems()){
		if(bit.getBook().getId() == b.getId()){
		    found = true;
		}
	    }
		...
		for(BookItem bit: bo.getItems()){
			if(bit.getBook().getId() == b.getId()){
			   bit.setQuantity(bit.getQuantity() + 1);
			   bo.setOrderTotal(bo.getOrderTotal() + bit.getBook().getPrice());
			}
		    }
	    } else {
		BookItem bi = new BookItem();
		...
		bi.setQuantity(1);
		bo.getItems().add(bi);
		bo.setOrderTotal(bo.getOrderTotal() + bi.getBook().getPrice());
	    }
	}
	BookOrder yetAnoBo = boRepo.save(bo);
	
	Customer anotherC = cRepo.save(thisCustomer);
	model.addAttribute("order", yetAnoBo);
	model.addAttribute("customer", anotherC);
    ...
    public String viewCart(Model model, HttpServletRequest req, @PathVariable Integer id){
	Account acc = AccountResolver.INSTANCE.getAccount(req);
	BookOrder bo = boRepo.findOne(id);
	bo.setNewOrder(false);
	boRepo.save(bo);
